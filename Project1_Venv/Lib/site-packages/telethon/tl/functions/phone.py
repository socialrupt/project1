...
class EditGroupCallParticipantRequest(TLRequest):
    ...
    def _bytes(self):
        return b''.join((
            b'\x80\xebu\xd9',
            struct.pack('<I', (0 if self.muted is None or self.muted is False else 1) | (0 if self.volume is None or self.volume is False else 2) | (0 if self.raise_hand is None or self.raise_hand is False else 4)),
            self.call._bytes(),
...
class ToggleGroupCallSettingsRequest(TLRequest):
    ...
    def _bytes(self):
        return b''.join((
            b'=\xb4\xbbt',
            struct.pack('<I', (0 if self.reset_invite_hash is None or self.reset_invite_hash is False else 2) | (0 if self.join_muted is None or self.join_muted is False else 1)),
            self.call._bytes(),
